...
AGENT_NAME="Engenheiro de Automação"
API_KEY_VAR="NOTION_SMARTENVIOS_API_KEY"
PROGRESS_COMMENT_MIN="${ENG_AUTOM_PROGRESS_COMMENT_MIN:-30}"
DEFAULT_ETA_MIN="${ENG_AUTOM_DEFAULT_ETA_MIN:-25}"
MICRO_TARGET_SEC="${ENG_AUTOM_MICRO_TARGET_SEC:-30}"
MICRO_MIN_CONTEXT_CHARS="${ENG_AUTOM_MICRO_MIN_CONTEXT_CHARS:-350}"
MICRO_MAX_OPEN="${ENG_AUTOM_MICRO_MAX_OPEN:-12}"
...
if [[ -x "${RUNTIME_GUARD}" ]]; then
  ...
  if ! ocw_guard_acquire_lock "eng-automacao-deterministic" "${CRON_LOCK_STALE_SEC:-1200}"; then
    echo '{"ok":true,"action":"skipped_already_running","lock":"eng-automacao-deterministic"}'
    exit 0
  fi
...
context_file="/tmp/eng_automacao_context_${PAGE_ID}.txt"
...
if (( context_chars >= MICRO_MIN_CONTEXT_CHARS )); then
  is_complex_card=1
elif [[ "${title_lc}" == *"novo projeto"* || "${title_lc}" == *"projeto"* || "${title_lc}" == *"arquitet"* || "${title_lc}" == *"integra"* || "${title_lc}" == *"refator"* || "${title_lc}" == *"migr"* || "${title_lc}" == *"n8n"* || "${title_lc}" == *"workflow"* || "${title_lc}" == *"automação"* || "${title_lc}" == *"automacao"* || "${title_lc}" == *"retry"* || "${title_lc}" == *"timeout"* ]]; then
  is_complex_card=1
fi
...
create_micro_cards() {
  ...
  while (( i <= total )); do
    ...
    body_file="/tmp/eng_automacao_micro_${parent_id}_${i}.md"
    cat > "${body_file}" <<EOF
...
if [[ "${context_chars}" -lt 80 ]]; then
  "${HELPER}" comment "${PAGE_ID}" "${API_KEY_VAR}" "Pendência: contexto técnico insuficiente no body (mínimo 80 chars). Card devolvido para Diretor Tech detalhar escopo executável e evitar bloqueio em Impedimento." "Engenheiro de Automação" >/dev/null || true
...
if (( is_micro_card == 1 )); then
  evidence_file="/tmp/eng_automacao_micro_evidence_${PAGE_ID}.txt"
  {
...
if (( is_micro_card == 0 && is_complex_card == 1 && has_microplan_marker == 0 )); then
  ...
  n8n_workflow_id=""
  n8n_context="product"
  if (( is_n8n_card == 1 )); then
    n8n_ctx_src="$(printf '%s\n%s\n' "${TITLE}" "${context_text}" | tr '[:upper:]' '[:lower:]')"
    if grep -Eq 'finan|finance|fatur|boleto|conta|pagament|receb|tesour|fiscal' <<<"${n8n_ctx_src}"; then
      n8n_context="finance"
    elif grep -Eq 'sales|venda|comercial|lead|crm|pipedrive|pré-venda|pre-venda' <<<"${n8n_ctx_src}"; then
      n8n_context="sales"
    else
      n8n_context="product"
    fi
    n8n_workflow_id="$(printf '%s\n%s\n' "${TITLE}" "${context_text}" | grep -Eo '[A-Za-z0-9]{21}' | head -n 1 || true)"
    if [[ -z "${n8n_workflow_id}" ]]; then
      "${HELPER}" comment "${PAGE_ID}" "${API_KEY_VAR}" "Pendência obrigatória: não foi possível identificar o workflow_id do n8n no card. Inclua ID/URL do workflow para validação e execução via API nativa. Card movido para Impedimento até complemento objetivo do dado." "${AGENT_NAME}" >/dev/null || true
      "${HELPER}" update-agent "${PAGE_ID}" "${API_KEY_VAR}" "Diretor Tech" >/dev/null || true
      "${HELPER}" update-status "${PAGE_ID}" "${API_KEY_VAR}" "Impedimento" >/dev/null || true
      echo "{\"ok\":true,\"action\":\"needs_n8n_workflow_id\",\"page_id\":\"${PAGE_ID}\",\"status\":\"Impedimento\",\"agent\":\"Diretor Tech\"}"
      exit 0
    fi
    n8n_check_json="$("${SCRIPT_DIR}/n8n-api.sh" --context "${n8n_context}" can-write "${n8n_workflow_id}" 2>/dev/null || true)"
    n8n_can_write="$(jq -r '.can_write // false' <<<"${n8n_check_json}" 2>/dev/null || echo false)"
    n8n_http_code="$(jq -r '.http_code // 0' <<<"${n8n_check_json}" 2>/dev/null || echo 0)"
    if [[ "${n8n_can_write}" == "true" ]]; then
      n8n_api_check="ok_http_${n8n_http_code}"
      n8n_can_write=1
    else
      n8n_api_check="falha_http_${n8n_http_code}"
      n8n_can_write=0
    fi
    if (( n8n_can_write == 0 )); then
      "${HELPER}" comment "${PAGE_ID}" "${API_KEY_VAR}" "Bloqueio técnico: validação de escrita via API nativa do n8n falhou (contexto=${n8n_context}, workflow_id=${n8n_workflow_id}, ${n8n_api_check}). Verifique credencial do contexto (N8N_${n8n_context^^}_API_KEY). Card movido para Impedimento até correção da credencial/acesso." "${AGENT_NAME}" >/dev/null || true
      "${HELPER}" update-agent "${PAGE_ID}" "${API_KEY_VAR}" "Diretor Tech" >/dev/null || true
      "${HELPER}" update-status "${PAGE_ID}" "${API_KEY_VAR}" "Impedimento" >/dev/null || true
      echo "{\"ok\":true,\"action\":\"needs_n8n_api_write_access\",\"page_id\":\"${PAGE_ID}\",\"workflow_id\":\"${n8n_workflow_id}\",\"n8n_context\":\"${n8n_context}\",\"n8n_api_check\":\"${n8n_api_check}\",\"status\":\"Impedimento\",\"agent\":\"Diretor Tech\"}"
      exit 0
    fi
  fi
  ...
  if (( ${#micro_ids[@]} > 0 )); then
    marker_file="/tmp/eng_automacao_parent_marker_${PAGE_ID}.md"
    {
    ...
    if (( is_n8n_card == 1 )); then
      "${HELPER}" comment "${PAGE_ID}" "${API_KEY_VAR}" "Planejamento concluído: demanda fatiada em ${#micro_ids[@]} micro-cards (alvo: <=${MICRO_TARGET_SEC}s por fatia). Evidência API n8n: contexto=${n8n_context}, workflow_id=${n8n_workflow_id}, check=${n8n_api_check}. Execução seguirá por prioridade nas próximas rodadas." "${AGENT_NAME}" >/dev/null || true
    else
    ...
    "${HELPER}" update-status "${PAGE_ID}" "${API_KEY_VAR}" "Em andamento" >/dev/null || true
    echo "{\"ok\":true,\"action\":\"planned_microcards\",\"page_id\":\"${PAGE_ID}\",\"title\":$(printf '%s' "${TITLE}" | jq -Rs .),\"micro_count\":${#micro_ids[@]},\"micro_target_sec\":${MICRO_TARGET_SEC},\"n8n_context\":\"${n8n_context}\",\"n8n_api_check\":\"${n8n_api_check}\",\"workflow_id\":\"${n8n_workflow_id}\",\"status\":\"Em andamento\"}"
    exit 0
  fi
fi
