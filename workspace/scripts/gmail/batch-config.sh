...
export MAIL_BATCH_MIN_LIMIT="${MAIL_BATCH_MIN_LIMIT:-10}"        # Mínimo de emails por batch
export MAIL_BATCH_MAX_LIMIT="${MAIL_BATCH_MAX_LIMIT:-50}"        # Máximo (aumentar para 200 quando estável)
export MAIL_BATCH_STEP_UP="${MAIL_BATCH_STEP_UP:-5}"
export MAIL_BATCH_STEP_DOWN="${MAIL_BATCH_STEP_DOWN:-5}"
export MAIL_BATCH_SUCCESS_STREAK="${MAIL_BATCH_SUCCESS_STREAK:-2}"
export MAIL_BATCH_SLOW_RUN_SEC="${MAIL_BATCH_SLOW_RUN_SEC:-120}"  # Reduzido para detectar lentidão mais cedo

# Paralelismo no Gmail (batch-mark-read, batch-archive): mais = menos tempo total, mais carga na API
export GMAIL_BATCH_PARALLEL="${GMAIL_BATCH_PARALLEL:-8}"
...
export MAIL_EMERGENCY_BATCH_LIMIT="${MAIL_EMERGENCY_BATCH_LIMIT:-50}"   # Aumentado para drenagem rápida
export MAIL_EMERGENCY_FREQUENCY_MIN="${MAIL_EMERGENCY_FREQUENCY_MIN:-30}"
